...
- `score` 默认 single：一次模型请求只返回逐项匹配（含多处证据）、评论及面试问题，不提取姓名、电话等个人字段，也不生成独立事实目录。代码生成内部 ID、核验引用并统一算分。结构或来源校验失败时最多从原输入纠正一次。
- `score --pipeline jev`：生成模型分别整理简历证据和 JD，再由 Jev 判断，代码算分并用本地模板报告。
- 仅评分内部使用来源块，保留 PDF 提取文本行号。每处引用用 `block_id` 和 `end_block_id` 指定连续范围，不限制行数；单行可省略或留空 end_block_id。引文必须是该范围的连续原文，允许空白差异，不能跳行或改写。输出保留完整范围上下文。这些 b1/b2 编号由代码添加，不是 PDF 原生段落；extract 不使用它们。不设置证据或 JD 要求条数上限。单模型每项判断支持多处不连续引用，结果用 `evidence_ids` / `evidences` 数组表示；可选 Jev 保留旧版单条证据选择及标量字段。
