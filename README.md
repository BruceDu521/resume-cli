...
- 无效输出最多让模型根据原始输入和校验原因纠正 3 次（首次请求加重试共最多 4 次生成，仍受命令总超时限制）。不会把损坏的响应直接当指令；若纠正请求失败，保留初次校验原因。
...
Go 测试使用内存 HTTP 替身，AI/CLI 测试默认禁止真实网络，不读取 `.env`。覆盖文件与大小边界、参数范围/溢出、PDF 进程失败、完整输入、JSON 修复、最多三次纠正、厂商协议、超时取消、缓存、输出防覆盖及中英文界面/报告的独立性。`make check` 包含单测、race 和 vet。
