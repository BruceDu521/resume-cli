...
`score` 默认 single：本地解析 PDF/JD → 完整文本及六字段 Schema → 模型直接输出四项整数分数、评语及面试问题 → 本地校验 JSON/0–100/非空报告 → 返回 model-assessment-v1。模型无需生成引用、行号或中间匹配数组；score 不依赖 extract，也不复用其缓存。最多一次纠正，若纠正请求失败，错误保留初次校验原因及后续请求失败。
...
Document 保留完整 text/hash，以及带 ID/page/text 的非空行 Block。Resume 为题目要求的姓名、联系方式、城市、学历与技能。可选 Jev 内部 Candidate 带 Fact，供匹配使用工作及项目原文。普通 extract 直接返回 Resume，不再生成 Candidate。原先64条上限是没有充分验证的工程取值，已移除；保留总输入/响应字节上限。
...
Job 不限制 Requirement 条数，保留 JD 原文及必需/优先。共用领域结构支持多条独立引用，但可选 Jev 仍使用旧版单条证据选择。默认 single 不再使用这套引用合同。满足/部分/明确不符必须有证据，unknown 不声称证据。原文校验不证明引用充分，也不证明报告评论无误。
...
默认 model-assessment-v1：四项分数由模型基于完整输入评价，无固定加权公式。报告不伪装成逐项校验过的证据评分；不同模式/旧版本分数不能直接比较。

可选 Jev 策略 evidence-v1：满足100、部分50、明确不符0、unknown0；后两者在报告中区分。技能/经历/教育权重50%/35%/15%，必需2、优先1；缺失维度剔除分母并标 not_required。模型给出的分值按状态重新计算，不让它修改权重或直接定总分。
