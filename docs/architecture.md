...
合成样例以 `samples.go` 的 `go:embed` 嵌入二进制；`samples <新目录>` 导出固定四份中英文 PDF/JD，不访问网络，也不依赖 Poppler。目标目录已存在则拒绝，避免覆盖使用者内容。
...
两项 AI 任务共用 Generator 和 decodeChecked：正常一次生成；无效 JSON 或字段校验失败最多再纠正 3 次（首次请求加重试共最多 4 次生成，仍受命令总超时限制），使用原输入和固定校验原因，不回传未信任响应。纠正请求失败时保留初次校验原因。所有调用均记录用量。
...
默认 PDF 32 MiB、文本 64 KiB、JD 32 KiB，分别通过 --max-pdf-mib、--max-text-kib、--max-jd-kib 配置。参数必须是正整数；最大分别为 200 MiB、256 KiB、128 KiB，并校验乘法溢出；在读取阶段执行限制。移除旧的固定序列化输入 200 KiB 检查，避免 JSON 转义和配置放大后出现隐藏上限。AI 响应仍限 2 MiB。超过资源限制明确报错，不按技能或要求条数截断。HTTP 单次 60s，命令默认 90s；429/529/502/503/504 最多三次，取消传播，不重试不明网络故障或 401。HTTPS 且禁止重定向。
