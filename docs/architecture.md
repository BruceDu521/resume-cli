...
默认 PDF 100 MiB、文本 128 KiB、JD 64 KiB，分别通过 --max-pdf-mib、--max-text-kib、--max-jd-kib 配置。参数校验正整数和乘法溢出；在读取阶段执行限制。移除旧的固定序列化输入 200 KiB 检查，避免 JSON 转义和配置放大后出现隐藏上限。AI 响应仍限 2 MiB。超过资源限制明确报错，不按技能或要求条数截断。HTTP 单次 60s，命令默认 90s；429/529/502/503/504 最多三次，取消传播，不重试不明网络故障或 401。HTTPS 且禁止重定向。
