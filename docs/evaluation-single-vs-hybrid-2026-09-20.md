...
| API key | RESUME_AI_API_KEY（Jev 单独使用 TYPESAFE_API_KEY） | 不支持命令行明文 key |
...
- “整例规则通过”指一次试验的要求类别、必需/优先、状态、证据及基本报告检查全部通过。规则来自合成输入，差异逐项阅读核对，不是独立人工标注准确率。一项不符即整例未通过；20/32 不代表只有 62.5% 的字段正确。通过数越高仅说明更符合当前预设规则。

## 结果

| 路径 | 成功输出 | 整例规则通过数 | 中位耗时（含失败） | 范围 | 每次尝试平均估算费用 |
| --- | ---: | ---: | ---: | ---: | ---: |
