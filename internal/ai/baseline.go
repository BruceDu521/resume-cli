...
func (s Structurer) Evaluate(ctx context.Context, d domain.Document, jd, lang string) (domain.Candidate, domain.Job, []domain.Judgment, string, []string, error) {
	...
	e := s.decodeChecked(ctx, Request{"baseline", safety + "\n" + requirementRules + "\nIndependently extract the candidate and JD requirements, then judge each requirement. Candidate facts must quote a numbered document block, have unique IDs and category skill/experience/education. Retain full context including negation. JD text must be verbatim and requirements have unique IDs. Each judgment references a requirement and a supporting/contradicting fact ID. Status satisfied=100, partial=50, unmet=0, unknown=0. Satisfied requires evidence of the requested level; personal projects can establish basic familiarity, but do not automatically establish professional experience. Partial means relevant evidence establishes some but not all required duration, scope or depth. Unmet requires explicit contradiction or denial; missing evidence is unknown, not inability. A shorter documented role is relevant partial evidence, not proof of total experience below a minimum. Never sum overlapping employment periods. Unknown has empty evidence_id; all other states require evidence. Include all judgments. Do not compute the overall score. Write a concise comment and 1-3 interview questions in " + lang + ". Do not infer skill years from total employment duration.", map[string]any{"resume_blocks": d.Blocks, "jd": jd}, schema}, &v, validate)
	for i := range v.Judgments {
		v.Judgments[i].Score = map[string]float64{"satisfied": 100, "partial": 50, "unmet": 0, "unknown": 0}[v.Judgments[i].Status]
	}
